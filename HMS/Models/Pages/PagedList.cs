...
namespace HMS.Models.Pages
{
	public class PagedList<T> : List<T>
	{
        ...
        private static IQueryable<T> Search<T>(IQueryable<T> query, string propertyName, string searchTerm)
        {
            ...
            {
                // Виклик Contains для рядка
                body = Expression.Call(source, nameof(string.Contains), Type.EmptyTypes,
                    Expression.Constant(searchTerm, typeof(string)));
            }
            else if (source.Type == typeof(int))
            {
                // Парсимо searchTerm у число
                if (int.TryParse(searchTerm, out var number))
                {
                    body = Expression.Equal(source, Expression.Constant(number));
                }
                else
                {
                    // Якщо не вдалося конвертувати, повертаємо без фільтрації
                    return query;
                }
            }
            else
